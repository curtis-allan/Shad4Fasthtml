...
select_cls = {"trigger": "flex h-10 w-full items-center justify-between rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 [&>span]:line-clamp-1",
...
def Portal(*c, id=None, **kwargs):
    if not id:
        raise ValueError("`id` is required")
    return Div(*c, id=f"{id}-portal", style="position:fixed;left:0;top:0;display:none;", **kwargs)
